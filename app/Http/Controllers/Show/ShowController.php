...
class ShowController extends Controller{
	...
	public function worker( $id ) {
		$worker = Worker::where( 'id', $id )
		                ...
		                ->first();
		if ( $worker != null ) {
			return view( 'show.worker', [ 'worker' => $worker ] );
		} else {
	...
	public function paymentMethod() {
		$payment_methods = Payment_method::where( 'id', '<>', 14 )
		                                 ->where( 'id', '<>', 13 )
		                                 ->orderBy( 'title', 'asc' )
		                                 ->get();
		if ( count( $payment_methods ) == 0 ) {
			...
			return view( 'show.payment-methods', [ 'payment_methods' => $payment_methods ] );
		}
	}
	...
	public function paymentType() {
		...
		if ( count( $payment_types ) == 0 ) {
			...
			return view( 'show.payment-types', [ 'payment_types' => $payment_types ] );
		}
	}

	public function marketingSource() {
		$marketing_sources = Marketing_source::orderBy( 'title', 'asc' )->get();
		if ( count( $marketing_sources ) == 0 ) {
			return redirect( '/marketing-source' );
		} else {
			return view( 'show.marketing-sources', [ 'marketing_sources' => $marketing_sources ] );
		}
	}
	...
	public function transaction( $id ) {
		...
		if ( isset( $transaction ) && $transaction != null ) {
			if ( $transaction->collected == 1 ) {
				Session::flash( 'message_success', $transaction->case_id . ' Collected' );
			}
	...
	public function search( Request $request ) {
		...
		if ( isset( $request->collected ) && $request->collected == "on" ) {
			$collected = "checked";
		} else {
			$collected = null;
		}
		...
		if ( isset( $request->marketing_source_id ) ) {
		...
		if ( $collected != null || $case_id != null || $client_name != null || $marketing_source_id != null || $amount_due != null || $total_price != null || $start_date != null || $end_date != null || $payment_method_id != null || $payment_type_id != null || $worker_id != null ) {
			...
			if ( $collected != null ) {
				$query->where( 'transaction_detalis.payed', 1 );
			}
			if ( $case_id != null ) {
				$query->where( 'transactions.case_id', $case_id );
			}
			if ( $client_name != null ) {
				$query->where( 'transactions.client_name', $client_name );
			}
			if ( $marketing_source_id != null ) {
				$query->where( 'transactions.marketing_source_id', $marketing_source_id );
			}
			if ( $total_price != null ) {
				$query->where( 'transactions.total_price', $total_price );
			}
			if ( $amount_due != null ) {
				$query->where( 'transaction_detalis.amounts_due', $amount_due );
			}
			if ( $start_date != null ) {
				$query->whereDate( 'transaction_detalis.payment_date', '>=', $start );
			}
			if ( $end_date != null ) {
				$query->whereDate( 'transaction_detalis.payment_date', '<=', $end );
			}
			if ( $payment_method_id != null ) {
				$query->where( 'transaction_detalis.payment_method_id', $payment_method_id );
			}
			if ( $payment_type_id != null ) {
				$query->where( 'transaction_detalis.payment_type_id', $payment_type_id );
			}
			if ( $worker_id != null ) {
				$query->where( 'transaction_detalis.worker_id', $worker_id );
			}
			...
			foreach ( $results as $key => $val ) {
				...
				if ( $collected != null ) {
					$detalis->where( 'transaction_detalis.payed', 1 );
				}
				if ( $amount_due != null ) {
					$detalis->where( 'transaction_detalis.amounts_due', $amount_due );
				}
				if ( $start_date != null ) {
					$detalis->whereDate( 'transaction_detalis.payment_date', '>=', $start );
				}
				if ( $end_date != null ) {
					$detalis->whereDate( 'transaction_detalis.payment_date', '<=', $end );
				}
				if ( $payment_method_id != null ) {
					$detalis->where( 'transaction_detalis.payment_method_id', $payment_method_id );
				}
				if ( $payment_type_id != null ) {
					$detalis->where( 'transaction_detalis.payment_type_id', $payment_type_id );
				}
				if ( $worker_id != null ) {
					$detalis->where( 'transaction_detalis.worker_id', $worker_id );
				}
	...
	public function report( Request $request ) {
		...
		if ( $start_date != null ) {
			...
			$result_marketing_query = Transaction_detalis::select( 'amounts_due', 'marketing_sources.title as msTitle', 'payment_method_id' )
			...
			$for_marketing       = [];
			$result_marketing       = [];
			foreach ( $result_marketing_query as $key => $val ) {
				if ( $val->payment_method_id != 14 && $val->payment_method_id != 13 ) {
					if( !in_array($val->msTitle, $for_marketing) ){
						$for_marketing[$val->msTitle] = $val->msTitle;
						$result_marketing[ $val->msTitle ] = $val->amounts_due;
					}
					else{
						$result_marketing[ $val->msTitle ] += $val->amounts_due;
					}
			...
			$result_worker_query  = Transaction_detalis::select( 'amounts_due', 'worker_id',
				'first_name', 'last_name', 'payment_method_id',
			...
			foreach ( $result_worker_query as $key => $val ) {
				if ( $val->payment_method_id != 14 && $val->payment_method_id != 13 ) {
					if( !in_array($val->worker_id, $fw) ){
						$fw[$val->worker_id] = $val->worker_id;
						$result_worker[ $val->worker_id ] = $val->amounts_due;
						$name[ $val->worker_id ]          = $val->first_name . ' ' . $val->last_name;
					}
					else{
						$result_worker[ $val->worker_id ] += $val->amounts_due;
					}
			...
			$new_bissness_new_deal = Transaction_detalis::select( 'amounts_due', 'payment_type_id', 'payment_method_id',
			...
			foreach ( $new_bissness_new_deal as $key => $val )
			{
				if ( $val->payment_method_id != 14 && $val->payment_method_id != 13 )
				{
					...
					if ( $val->payment_type_id == 1 || $val->payment_type_id == 4 || $val->payment_type_id == 6 ) {
						if( !in_array($val->worker_id, $b) ){
							$b[$val->worker_id] = $val->worker_id;
					...
					if ( $val->payment_type_id == 1 || $val->payment_type_id == 4 ) {
						$ab = $ab + 1;
						if ( !in_array($val->worker_id,$a) ) {
							$a[$val->worker_id] = $val->worker_id;
		...
		if ( isset( $request->exel ) ) {
			...
			return view( 'create.saler-report', compact( 'new_deal', 'new_bissness_total', 'new_bissness', 'total_amout', 'workingDays', 'result_payment_method', 'result_payment_type', 'result_worker', 'result_marketing', 'end_date', 'start_date', 'name' ) );
		}

		return view( 'show.report', compact( 'new_deal', 'new_bissness_total', 'new_bissness', 'total_amout', 'workingDays', 'result_payment_method', 'result_payment_type', 'result_worker', 'result_marketing', 'end_date', 'start_date', 'name' ) );
	}
